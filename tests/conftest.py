...
flags:
    ...
    python -m pytest ./projects/roman_real/tests --high=1 --mask=ones
...
# example1.mask scale cuts of the frozen contract, and "ones" keeps
# every data point (no scale cuts). Each maps to a frozen TATT
# dataset variant differing only in its mask_file line (the naming
# rule lives with fastpt_masks in cocoa_testing.CocoaTestHarness).
...
def pytest_addoption(parser):
    ...
    16-18) at the frozen default settings; --high=1 repeats them
    ...
    example1.mask scale cuts, and --mask=ones keeps every data
