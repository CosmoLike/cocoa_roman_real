"""Unit tests 16-18: cfastpt vs python FAST-PT, compared directly.
...
16. example1 (cosmic shear): the SAME 30 hard-coded points
     ...
     30 points cheap.
17. example2 (3x2pt): the same three-block sweep as test 16 on the
     ...
     data vector.
18. example2_2x2pt (2x2pt): the same sweep on the 2x2pt likelihood
...
example1.mask scale cuts, and --mask=ones keeps every data point
(no scale cuts), the strictest comparison; the 0.2 pass rule
applies unchanged:

    python -m pytest ./projects/roman_real/tests/test_fastpt.py --mask=ones

On this project the 3x2pt sweep does not run under --mask=ones:
cosmolike rejects the all-ones covariance as not positive definite
(the README records the measurement).
...
class TestCfastptVsFastptSweep(unittest.TestCase):
    """Tests 16-18, the direct CFASTPT-vs-FASTPT comparison.
    ...
    def test_x16_cfastpt_vs_fastpt_sweep(self):
        ...
        # "frozen" defaults keeps a run outside pytest on the
        # default settings unless the variables are exported by hand
    ...
    def test_x17_cfastpt_vs_fastpt_sweep_3x2pt(self):
        """3x2pt: cfastpt and FASTPT agree at the same 30 IA points.

        Test 16 on example2: the same three blocks, the same points,
    ...
    def test_x18_cfastpt_vs_fastpt_sweep_2x2pt(self):
        """2x2pt: cfastpt and FASTPT agree at the same 30 IA points.

        Test 16 on example2_2x2pt: the same three blocks, the same
