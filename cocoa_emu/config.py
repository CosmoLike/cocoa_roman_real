#Config file for cosmic_shear emulator
import yaml
import numpy as np
import os
...
class cocoa_config:
    ...
    def __init__(self, config_file):
        ...
        self.params = config_args['params']     # list of params, used to create arrays and dicts

        self.load_lkl(config_lkl)
        self.load_params(self.params)
        #self.load_emu(config_args['emulator']) # load the emulator config
    ...
    def load_lkl(self, lkl_args):
        ...
        also open .dataset file (default is example1.dataset, but this is not a good choice for most applications)
        '''
        ...
        try:
            ...
            print('  > using "example1.dataset" (NOT recommended for training)')
            self.data_file = 'example1.dataset'
    ...
    def load_params(self, param_args):
        ...
        for param in params_list:
            ...
            if('value' not in keys and 'derived' not in keys and len(keys)>1):
                ...
                self.priors[param] = param_args[param]['prior']

        return

    def load_emu(self, emu_args):
        """
        Load emulator configuration from the YAML file.
        """
        self.emu_name = emu_args['name']
        self.n_sample_dims = len(self.running_params)
        self.n_emcee_walkers = emu_args.get('n_emcee_walkers', 100)
        self.n_mcmc = emu_args.get('n_mcmc', 1000)
        self.n_burn_in = emu_args.get('n_burn_in', 100)
        self.n_thin = emu_args.get('n_thin', 10)
        self.chainname = emu_args.get('chainname', 'chain')
        self.chaindir = emu_args.get('chaindir', 'chains')
