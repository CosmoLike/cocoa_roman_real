...
class Config:
    ...
    def load_lkl(self, config_args_lkl):
        ...
        self.get_cov(pjoin(dst,dataset["cov_file"]),self.mask_lkl)
        #invcov = np.linalg.inv(self.get_cov[self.mask_lkl][:,self.mask_lkl])
        self.dv_std = np.sqrt(np.diagonal(self.cov_inv_masked))
        self.masked_inv_cov = self.cov_inv_masked
        self.inv_cov = self.cov_inv
        """
    ...
    def load_emu(self, config_args_emu):
        ...
        if 'training' in config_args_emu:
        ...
        if 'sampling' in config_args_emu:
