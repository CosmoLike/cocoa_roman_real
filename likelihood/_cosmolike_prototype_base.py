...
import cosmolike_roman_interface as ci
...
class _cosmolike_prototype_base(DataSetLikelihood):
  ...
  def set_source_related(self, **params_values):
    ci.set_nuisance_shear_calib(
      M = [
        params_values.get(p, None) for p in [
          "roman_M"+str(i+1) for i in range(self.source_ntomo)
        ]
      ]
    )
    ...
    ci.set_nuisance_shear_photoz(
      bias = [
        params_values.get(p, None) for p in [
          "roman_DZ_S"+str(i+1) for i in range(self.source_ntomo)
        ]
      ]
    )
    ...
    ci.set_nuisance_ia(
      A1 = [
        params_values.get(p, None) for p in [
          "roman_A1_"+str(i+1) for i in range(self.source_ntomo)
        ]
      ],
      A2 = [
        params_values.get(p, None) for p in [
          "roman_A2_"+str(i+1) for i in range(self.source_ntomo)
        ]
      ],
      B_TA = [
        params_values.get(p, None) for p in [
          "roman_BTA_"+str(i+1) for i in range(self.source_ntomo)
        ]
      ],
    )
  ...
  def set_lens_related(self, **params_values):
    ci.set_nuisance_bias(
      B1 = [
        params_values.get(p, None) for p in [
          "roman_B1_"+str(i+1) for i in range(self.lens_ntomo)
        ]
      ],
      B2 = [
        params_values.get(p, None) for p in [
          "roman_B2_"+str(i+1) for i in range(self.lens_ntomo)
        ]
      ],
      B_MAG = [
        params_values.get(p, None) for p in [
          "roman_BMAG_"+str(i+1) for i in range(self.lens_ntomo)
        ]
      ]
    )
    ci.set_nuisance_clustering_photoz(
      bias = [
        params_values.get(p, None) for p in [
          "roman_DZ_L"+str(i+1) for i in range(self.lens_ntomo)
        ]
      ]
    )
    ci.set_point_mass(
      PMV = [
        params_values.get(p, None) for p in [
          "roman_PM"+str(i+1) for i in range(self.lens_ntomo)
        ]
      ]
    )

  #  self.baryon_pcs_qs[0] = params_values.get("roman_BARYON_Q1", 0.0)
