...
class _cosmolike_prototype_base(DataSetLikelihood):
    def initialize(self, probe):
        ...
        self.z_interp_1D = np.concatenate(
            (
                np.linspace(0.0, 3.0, max(100, int(0.80 * tmp))),
                np.linspace(3.0, 50.1, max(100, int(0.40 * tmp))),
                np.linspace(1070, 1100, max(50, int(0.10 * tmp))),
            ),
            axis=0,
        )
        ...
        self.z_interp_2D = np.concatenate(
            (
                np.linspace(0, 3.0, max(50, int(0.75 * tmp))),
                np.linspace(3.01, 50.1, max(30, int(0.25 * tmp))),
            ),
            axis=0,
        )
        self.len_z_interp_2D = len(self.z_interp_2D)

        self.log10k_interp_2D = np.linspace(
            -4.99, 2.0, int(1250 + 250 * self.accuracyboost)
        )
        ...
        if self.use_emulator:
            ...
            ci.init_accuracy_boost(
                accuracy_boost=0.35, integration_accuracy=-1
            )  # seems enough to compute PM
        else:
            ci.init_ntable_lmax(lmax=int(self.lmax))

            ci.init_accuracy_boost(
                accuracy_boost=self.accuracyboost,
    ...
    def get_requirements(self):
        if self.use_emulator:
            if self.probe == "xi":
                ...
                return {"cosmic_shear": None, "wtheta": None}
            ...
            # JVR NOTE: our likelihood must communicate with the baryons theory which (k,z) values to compute the baryon suppression factor
            # NOTE: log10k_interp_2D is in 1/Mpc, the baryons theory must do the conversion if necessary
            if self.external_baryon_suppression:
    ...
    def set_cosmo_related(self):
        ...
        if not self.use_emulator:
            PKL = self.provider.get_Pk_interpolator(
                ("delta_tot", "delta_tot"),
                nonlinear=False,
                extrap_kmax=2.5e2 * self.accuracyboost,
            )
            lnPL = PKL.logP(
                self.z_interp_2D, np.power(10.0, self.log10k_interp_2D)
            ...
            if self.non_linear_emul == 1:
                ...
                tmp = interp1d(
                    ...
                    assume_sorted=True,
                )(self.log10k_interp_2D - np.log10(h))  # h/Mpc
                ...
                lnPNL = self.provider.get_Pk_interpolator(
                    ("delta_tot", "delta_tot"),
                    nonlinear=True,
                    extrap_kmin=1e-6,
                    extrap_kmax=2.5e2 * self.accuracyboost,
                ).logP(self.z_interp_2D, np.power(10.0, self.log10k_interp_2D)).flatten(
                    order="F"
                ) + np.log(h**3)
                # on z < 10.0, replace it with Halofit
                lnPNL = np.where(
                    (self.z_interp_2D < 10)[:, None],
                    lnPL.reshape(
                        self.len_z_interp_2D, self.len_log10k_interp_2D, order="F"
                    )
                    + lnbt,
                    lnPNL.reshape(
                        self.len_z_interp_2D, self.len_log10k_interp_2D, order="F"
                ...
                raise LoggedError(
                    self.log,
                    "non_linear_emul = %d is an invalid option",
                    self.non_linear_emul,
                )
            ...
            ci.set_cosmology(
                ...
                chi=self.provider.get_comoving_radial_distance(self.z_interp_1D)
                * h,  # convert to Mpc/h
            )
        else:
            ci.set_distances(
    ...
    def set_source_related(self, **params):
        ...
        ci.set_nuisance_shear_calib(
            M=[
                params.get(p, 0)
                for p in [survey + "_M" + str(i + 1) for i in range(ntomo)]
            ]
        )
        if not self.use_emulator:
            if self.external_nz_modeling:
                ...
                ci.set_nuisance_shear_photoz(
                    bias=[
                        params.get(p, 0)
                        for p in [survey + "_DZ_S" + str(i + 1) for i in range(ntomo)]
                    ]
                )
            else:
                ci.set_nuisance_shear_photoz(
            ...
            ci.set_nuisance_ia(
                A1=[
                    params.get(p, 0)
                    for p in [survey + "_A1_" + str(i + 1) for i in range(ntomo)]
                ],
                A2=[
                    params.get(p, 0)
                    for p in [survey + "_A2_" + str(i + 1) for i in range(ntomo)]
                ],
                B_TA=[
                    params.get(p, 0)
                    for p in [survey + "_BTA_" + str(i + 1) for i in range(ntomo)]
                ],
            )
    ...
    def set_lens_related(self, **params):
        ...
        ci.set_point_mass(
            PMV=[
                params.get(p, 0)
                for p in [survey + "_PM" + str(i + 1) for i in range(ntomo)]
            ]
        )
        if not self.use_emulator:
            ci.set_nuisance_bias(
                B1=[
                    params.get(p, 1)
                    for p in [survey + "_B1_" + str(i + 1) for i in range(ntomo)]
                ],
                B2=[
                    params.get(p, 0)
                    for p in [survey + "_B2_" + str(i + 1) for i in range(ntomo)]
                ],
                B_MAG=[
                    params.get(p, 0)
                    for p in [survey + "_BMAG_" + str(i + 1) for i in range(ntomo)]
                ],
            )
            if self.external_nz_modeling:
                # here we send n(z) at every point in the chain as the user may
                ...
                ci.set_nuisance_clustering_photoz(
                    bias=[
                        params.get(p, 0)
                        for p in [survey + "_DZ_L" + str(i + 1) for i in range(ntomo)]
                    ]
                )
            else:
                ci.set_nuisance_clustering_photoz(
    ...
    def internal_get_datavector_emulator(self, **params):
        ...
        PM = [
            params.get(p, 0)
            for p in [survey + "_PM" + str(i + 1) for i in range(self.lens_ntomo)]
        ]
        if self.probe not in ("xi", "xi_gg") and not all(v == 0 for v in PM):
            self.set_lens_related(**params)
            cosmo_status = self.set_cosmo_related()
            if cosmo_status is not None and np.isinf(cosmo_status):
                return None
        ...
        if self.probe == "xi":
            ...
            if len(tmp1) != sizes[0] or len(tmp2) != sizes[1]:
            ...
            if len(tmp1) != sizes[0] or len(tmp2) != sizes[1] or len(tmp3) != sizes[2]:
            ...
            tmp1 = self.provider.get_cosmic_shear()
            tmp3 = self.provider.get_wtheta()
            if len(tmp1) != sizes[0] or len(tmp3) != sizes[2]:
                raise ValueError(f"Incompatible Sizes (Emulator 3x2pt)")
            istart = 0
            iend = sizes[0]
            dv[istart:iend] = tmp1[0 : sizes[0]]
            ...
            tmp2 = self.provider.get_ggl()
            tmp3 = self.provider.get_wtheta()
            if len(tmp2) != sizes[1] or len(tmp3) != sizes[2]:
                raise ValueError(f"Incompatible Sizes (Emulator 3x2pt)")
            istart = sizes[0]
            iend = sizes[0] + sizes[1]
            dv[istart:iend] = tmp2[0 : sizes[1]]
        ...
        if not self.use_baryon_pca:
            if not all(v == 0 for v in PM):
                dv = ci.compute_add_fpm_3x2pt_real_any_order(
                    datavector=dv, force_exclude_pm=0
                )
            else:
                dv = ci.compute_add_fpm_3x2pt_real_any_order(
                    datavector=dv, force_exclude_pm=1
                )
        else:
            Q = [
                params.get(p, 0)
                for p in [survey + "_BARYON_Q" + str(i + 1) for i in range(self.npcs)]
            ]
            if not all(v == 0 for v in PM):
                dv = ci.compute_add_fpm_3x2pt_real_any_order_with_pcs(
                    datavector=dv, Q=Q, force_exclude_pm=0
                )
            else:
                dv = ci.compute_add_fpm_3x2pt_real_any_order_with_pcs(
                    datavector=dv, Q=Q, force_exclude_pm=1
                )
    ...
    def internal_get_datavector(self, **params):
        ...
        if self.create_baryon_pca:
            pcs = ci.compute_baryon_pcas(
                scenarios=self.baryon_pca_select_sims, allsims=self.allsims
            )
