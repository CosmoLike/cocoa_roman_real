                ...
                if ax is not None:
                    ...
                    for idx, constraints in enumerate(constraints_list):
                        if constraints is not None:
                            ...
                            best_fit_str = f"{round(best_fit,3):.3f}"
                            sigma_lower_str = f"{round(sigma1_lower,3):.3f}"
                            sigma_upper_str = f"{round(sigma1_upper,3):.3f}"
                            #text = f"{chain_name}: {best_fit_str}$^{{+{sigma_upper_str}}}_{{-{sigma_lower_str}}}$"
                            text = f"{best_fit_str}$^{{+{sigma_upper_str}}}_{{-{sigma_lower_str}}}$"
                    ...
                    for k, (text, color) in enumerate(text_lines):
                        y_text = y_top + 0.0 + 0.05 * (len(text_lines)-k-1)
                        g.fig.text(x_center, y_text, text, ha='center', va='bottom', fontsize=30, color=color)
...
def main():
    ...
    v1_chain = 'chains/dc1_3x2'
    v2_chain = 'chains/dc1_alt2_v3'
    v3_chain = 'chains/dc1_alt3_v3'
    v4_chain = 'chains/dc1_alt4_v3'
    v5_chain = 'chains/dc1_alt5_v3'
    v6_chain = 'chains/dc1_alt6_v3'


    #Alternate dark energy
    plot_alts(chain_dir,[v1_chain,v2_chain,v3_chain],['Baseline ($w=-1.0$)', '$w=-0.9$', '$w_0=-0.9, w_a=0.5$'],
    analysis_settings,params_cosmo,output_filename='dc1_alt_DE.png')

    #Alternate baryonic models
    plot_alts(chain_dir,[v1_chain,v5_chain,v6_chain],['Baseline (No Baryons)', 'Baryons=Illustris', 'Baryons=Eagle-1'],
    analysis_settings,params_cosmo,output_filename='dc1_alt_baryons.png')

    #EuclidEmu2
    plot_alts(chain_dir,[v1_chain,v4_chain],['Baseline (No Baryons)', 'EuclidEmu2'],analysis_settings,params_cosmo,output_filename='dc1_alt_euclidemu2.png')
